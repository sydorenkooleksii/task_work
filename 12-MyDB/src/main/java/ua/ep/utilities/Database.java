...
public class Database {
    ...
    List<StudentHasCourse> studentHasCourseList = new ArrayList<>();
    ...
    public void getTableStudentHasCourse() throws SQLException {
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/driving_courses?useSSL=false", "root", "1234");

        try (PreparedStatement query = connection.prepareStatement("SELECT * FROM student_has_course")) {
            ResultSet rs = query.executeQuery();
            while (rs.next()) {
                StudentHasCourse studentHasCourse = new StudentHasCourse(rs.getInt("Student_id_Student"),
                        rs.getInt("Course_id_Course"));
                studentHasCourseList.add(studentHasCourse);
            }
        }
    }
    ...
    public String toString() {
        return "Database{" +'\n'+
                ...
                "studentHasCourseList=" + studentHasCourseList +'\n'+
